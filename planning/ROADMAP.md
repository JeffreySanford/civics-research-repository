...
The Maps workspace will move from a flat control list toward grouped, expandable layer categories while preserving independent layer toggles and the existing non-canvas information model.

Introduce a typed research spatial-coverage model that can express authoritative:

- administrative areas;
- points;
- bounding boxes;
- later polygons/geometries.

Spatial coverage must retain provenance and derivation method. Publisher or institution location is not research coverage and must never be substituted silently.

Discovery-to-map integration should use bounded summary/feature APIs rather than sending unbounded search hits to MapLibre. Every meaningful mapped value must remain available in semantic HTML through the same shared application state.
