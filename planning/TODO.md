...
- [x] Relevance tuning: field boosts (title^5, geography^4, subjects/program/authors^3, summary^2, citation^1, publisher^0.5), phrase boosts via `pf`/`pf2`, and `mm=2<67%`. Publisher was the clearest problem: every object is published by "U.S. Census Bureau", so an unweighted publisher field scored identically on every query. "North Dakota workforce" went from a broad match to 5 results with the LODES workplace dataset first.
...
- [x] **Make the existing adapters enumerate: 5 objects -> 112, with no new adapters.** `harvest()` returns every object a source publishes, defaulting to the single representative so the eleven national programs need no extra code. TIGER/Line and LODES read their objects from the generated catalog through `CatalogMetadataReader`, which removes the hardcoded constants that let the CPS adapter drift from the catalog in the first place. Measured: 112 of 181 objects now carry a DSpace UUID, at about 0.15s per object and roughly 24s of startup sync. `civics.sync.max-objects-per-source` caps it per run.
- [x] The representative object leads the harvest, so a cap can never drop the object the demo is built around. LODES starts at Alabama in catalog order; North Dakota was outside the first twenty-five.
- [x] One contract test over every registered adapter instead of one test per adapter. Verified it fails when the CPS drift is reintroduced.
- [x] **Nine more adapters: 112 -> 176.** ACS PUMS plus the eight single-object national programs, each a `CatalogBackedMetadataAdapter` subclass declaring a source and a program and nothing else. Required extending `SyncSource`, which had values for six sources only. All fourteen adapters reconcile at startup; 176 of 181 objects now carry a DSpace UUID.
- [x] Removed a third drift-prone copy while adding them: `firstSliceItemTitle` was a hand-maintained table of item titles that would have needed nine new lines. It only runs when a source has no adapter, where naming the source is the honest answer.
- [x] **Decided: the five LEHD research-package objects stay seed-only.** They are precisely the five without a DSpace UUID, and that is correct rather than a shortfall. Two working papers, a methodology report, a project and the restricted microdata: "this project has these parts" is a curatorial assertion, and no publisher endpoint reports it. Harvesting them would mean inventing a source for a claim the repository itself makes.
...
- [x] Workplace employment layer from the LODES WAC product, drawn as proportional circles rather than shading: one circle per county, radius interpolated on the square root of the job count so circle _area_ encodes the value. Live for every state, since WAC carries one row per workplace block rather than one per block pair (North Dakota 249 KB, California 6.3 MB against 97 MB for OD). Accessible equivalent is a ranked table with each county's share. The workforce view opens with both halves on.
