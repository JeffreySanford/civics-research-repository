...
The map-side request must preserve relevant criteria such as query, source system, publisher, program, research-object type, year/date and active projection identity/version.
...
This workstream does not render every C2 record as a point, geocode publisher addresses as research coverage, make MapLibre authoritative for spatial state, replace Discovery with a map-first search engine, or weaken the accessible HTML equivalent to accommodate canvas behavior.
