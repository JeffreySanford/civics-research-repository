...
function requireAcceptedSuite(suite) {
  if (suite?.experiment !== 'C2.1_ADVERSARIAL_STANDALONE') {
    throw new Error(
      'C2.1 report requires the adversarial standalone experiment.',
    );
  }
  if (
    suite?.kind !== 'measurement-suite' ||
    suite?.acceptedC21Evidence !== true
  ) {
    throw new Error(
      'C2.1 report requires an accepted measurement-suite artifact.',
    );
  }
  ...
  if (suite?.semanticAdmissionTimingExcluded !== true) {
    throw new Error(
      'C2.1 report requires semantic-admission timing exclusion.',
    );
  }
  ...
  if (
    Number(suite?.projectionObjectCount) !== C2_1_EXPECTED.projectionObjectCount
  ) {
    throw new Error(
      'C2.1 report requires exactly 1,000,181 projected objects.',
    );
  }
  ...
  if (
    !Array.isArray(suite?.restartBlocks) ||
    suite.restartBlocks.length !== C2_1_RESTART_BLOCKS
  ) {
    throw new Error(
      `C2.1 report requires exactly ${C2_1_RESTART_BLOCKS} restart blocks.`,
    );
  }
  if (
    !Array.isArray(suite?.workloadMatrix) ||
    suite.workloadMatrix.length < 21
  ) {
...
function requireSemanticAdmission(semantic) {
  if (
    semantic?.experiment !== 'C2.1_ADVERSARIAL_STANDALONE' ||
    semantic?.admitted !== true
  ) {
    throw new Error(
      'C2.1 report requires an admitted semantic-treatment artifact.',
    );
  }
  if (
    ...
    throw new Error(
      'C2.1 report requires semantic timings to remain discarded.',
    );
  }
  if (semantic?.projectionId !== C2_1_EXPECTED.projectionId) {
    throw new Error(
      'C2.1 semantic admission projection does not match the certified projection.',
    );
  }
  if (
    ...
    throw new Error(
      'C2.1 semantic admission object count does not match the certified projection.',
    );
  }
  if (semantic?.admittedTreatment !== C2_1_ADMITTED_TREATMENT) {
    throw new Error(
      'C2.1 semantic admission treatment does not match the measured treatment.',
    );
  }
  return semantic;
}
...
function requireSamples(benchmark, group) {
  ...
  if (
    !Array.isArray(solr) ||
    !Array.isArray(openSearch) ||
    solr.length === 0 ||
    solr.length !== openSearch.length
  ) {
    throw new Error(
      `${group} requires same-length non-empty paired raw samples.`,
    );
  }
  if (
...
function nearestRankSigned(values, percentile) {
  if (!Array.isArray(values) || values.length === 0) {
    throw new Error('At least one signed paired difference is required.');
  }
  if (!(percentile > 0 && percentile <= 1)) {
    throw new Error(
      'Signed paired-difference percentile must be greater than 0 and no greater than 1.',
    );
  }
  ...
  const sorted = values
    .map((value) => {
      if (!Number.isFinite(value)) {
        throw new Error('Signed paired difference must be finite.');
      }
      return value;
    })
...
function descriptivePair(solr, openSearch) {
  ...
  return {
    ...
    medianPairedDifferenceMs: nearestRankSigned(differences, 0.5),
    solrWinRatePercent:
...
function batchMetric(benchmark, group, blockId) {
  ...
  if (
    !Array.isArray(benchmark?.batchEvidence) ||
    benchmark.batchEvidence.length !== C2_1_BATCHES_PER_BLOCK
  ) {
    throw new Error(
      `restart block ${blockId} ${group} requires ${C2_1_BATCHES_PER_BLOCK} retained batches.`,
    );
  }
  return benchmark.batchEvidence.map((batch) => {
    if (
      ...
      throw new Error(
        `restart block ${blockId} batch ${batch?.batchId ?? 'missing'} has no retained sample indexes.`,
      );
    }
    const solr = batch.sampleIndexes.map((index) => samples.solr[index]);
    const openSearch = batch.sampleIndexes.map(
      (index) => samples.openSearch[index],
    );
    if (
      solr.some((value) => !Number.isFinite(value)) ||
      openSearch.some((value) => !Number.isFinite(value))
      ...
      throw new Error(
        `restart block ${blockId} batch ${batch.batchId} references an invalid sample index.`,
      );
    }
...
function summarizeBatchEffects(units, seed) {
  if (units.length !== EXPECTED_BATCH_UNITS) {
    throw new Error(
      `C2.1 batch inference requires exactly ${EXPECTED_BATCH_UNITS} independent batch summaries; found ${units.length}.`,
    );
  }
  ...
  return {
    ...
    openSearchWinRatePercent:
      Math.round((openSearchWins / units.length) * 10000) / 100,
    statistics,
    units,
  };
}
...
function semanticCellMap(semantic) {
  return new Map(
    (Array.isArray(semantic?.cells) ? semantic.cells : []).map((cell) => [
      cell.id,
      cell,
    ]),
  );
...
function workloadForBlock(block, id) {
  ...
  if (!workload) {
    throw new Error(
      `Restart block ${block?.blockId ?? 'missing'} is missing workload ${id}.`,
    );
  }
  if (
    workload?.benchmark?.projection?.projectionId !== C2_1_EXPECTED.projectionId
  ) {
...
function summarizeCell({ suite, semanticById, matrixCell, index }) {
  ...
  for (const block of suite.restartBlocks) {
    ...
    apiUnits.push(
      ...batchMetric(workload.benchmark, 'apiElapsed', block.blockId),
    );
    nativeUnits.push(
      ...batchMetric(workload.benchmark, 'engineReported', block.blockId),
    );
  }
  ...
  if (!semantic?.semantic?.admitted) {
    throw new Error(
      `${matrixCell.id}: semantic-admission cell is missing or not admitted.`,
    );
  }
  ...
  return {
    ...
    semantic: {
      ...
      facetCountsEqual:
        semantic.semantic.crossEngine?.facetCountsEqual === true,
...
function summarizeDirections(cells, metric) {
  ...
  return {
    solrLowerLatencyCells: directions.filter(
      (row) => row.direction === 'SOLR_LOWER_LATENCY',
    ).length,
    openSearchLowerLatencyCells: directions.filter(
      (row) => row.direction === 'OPENSEARCH_LOWER_LATENCY',
    ).length,
...
export function synthesizeC21Report({
  suite,
  semantic,
  now = () => new Date(),
} = {}) {
...
if (import.meta.url === pathToFileURL(process.argv[1] ?? '').href) {
  writeC21Report()
    .then(({ report, jsonPath, markdownPath }) => {
      ...
      console.log(
        `C2.1 statistical report Markdown written to ${markdownPath}`,
      );
      console.log(
